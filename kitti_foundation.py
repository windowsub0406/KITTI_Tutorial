...
class Kitti:
    ...
    def __init__(self, frame='all', velo_path=None, camera_path=None, \
                 img_type='gray', v2c_path=None, c2c_path=None):
...
class Kitti_util(Kitti):
    ...
    def __init__(self, frame='all', velo_path=None, camera_path=None, \
                 img_type='gray', v2c_path=None, c2c_path=None):
        super().__init__(frame, velo_path, camera_path, img_type, v2c_path, c2c_path)
    ...
    def __velo_filter(self, points):
        ...
        x_lim = self.__fov_setting(self.__x)[:,None]
        y_lim = self.__fov_setting(self.__y)[:,None]
        z_lim = self.__fov_setting(self.__z)[:,None]
        ...
        dist_lim = self.__fov_setting(self.__d)
    ...
    def __3d_in_range(self, points):
        ...
        return points[np.logical_and.reduce((self.__x > self.__y_range[0], self.__x < self.__y_range[1], \
                                             self.__y > self.__x_range[0], self.__y < self.__x_range[1], \
                                             self.__z > self.__z_range[0], self.__z < self.__z_range[1]))]

    def __fov_setting(self, points):
        """ filter points based on h,v FOV  """

        if self.__h_fov[1] == self.__h_max and self.__h_fov[0] == self.__h_min and \
                        self.__v_fov[1] == self.__v_max and self.__v_fov[0] == self.__h_min:
            return points
        if self.__h_fov[1] == self.__h_max and self.__h_fov[0] == self.__h_min:
            return points[self.__hv_in_range(self.__d, self.__z, self.__v_fov, fov_type='v')]
        elif self.__v_fov[1] == self.__v_max and self.__v_fov[0] == self.__v_min:
            return points[self.__hv_in_range(self.__x, self.__y, self.__h_fov, fov_type='h')]
        else:
    ...
    def __surround_view(self, points, depth):
        ...
        # filter points based on h,v fov
        x_img = self.__fov_setting(x_img)
        y_img = self.__fov_setting(y_img)
        dist = self.__fov_setting(self.__d)
        ...
        dist = self.__normalize_data(dist, min=0, max=120, scale=255, depth=depth)
    ...
    def __topview(self, points, scale):
        ...
        x_lim = self.__3d_in_range(self.__x)
        y_lim = self.__3d_in_range(self.__y)
        dist_lim = self.__3d_in_range(self.__d)

        x_size = int((self.__x_range[1] - self.__x_range[0]))
        y_size = int((self.__y_range[1] - self.__y_range[0]))
        ...
        # shift negative points to positive points (shift minimum value to 0)
        x_img += int(np.trunc(self.__x_range[1] * scale))
        y_img += int(np.trunc(self.__y_range[1] * scale))

        # normalize distance value & convert to depth map
        max_dist = np.sqrt((max(self.__x_range) ** 2) + (max(self.__y_range) ** 2))
        dist_lim = self.__normalize_data(dist_lim, min=0, max=max_dist, scale=255, depth=True)
        # array to img
        img = np.zeros([y_size * scale + 1, x_size * scale + 1], dtype=np.uint8)
        img[y_img, x_img] = dist_lim
        return img

    def velo_2_pano(self, h_fov, v_fov, depth=False):
    ...
    def velo_2_pano_frame(self, h_fov, v_fov, depth=False):
    ...
    def velo_2_topview(self, x_range, y_range, z_range, scale):
    ...
    def velo_2_topview_frame(self, x_range, y_range, z_range, scale):
...
def pano_example1():
    ...
    velo = Kitti_util(velo_path, frame=89)
...
def pano_example2():
    ...
    velo2 = Kitti_util(velo_path, frame='all')
...
def topview_example1():
    ...
    frame = velo.velo_2_topview_frame(x_range, y_range, z_range, scale)
...
def topview_example2():
    ...
    velo2 = Kitti_util(velo_path, frame='all')
    topview = velo2.velo_2_topview(x_range, y_range, z_range, scale)
